...
pub fn consume(
    ...
    try srch.update(DB, hash_kind, .patch, haxy_moment, arena.child_allocator, if (existing_maybe) |existing| srch.doc(&order_key, existing) else null, srch.doc(&order_key, record));
