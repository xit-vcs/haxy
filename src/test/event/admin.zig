...
test "user and repo" {
    ...
    try evt.consume(.server, .admin, .xit, repo_opts, io, allocator, &repo, evt.events_ref, &events_to_consume2);
    ...
    try evt.consume(.server, .admin, .xit, repo_opts, io, allocator, &repo, evt.events_ref, &[_]evt.EventWithId{.{
        .id = std.fmt.bytesToHex(repo_event_id, .lower),
        .author = author,
...
test "repos and users paginate newest first" {
    ...
    try evt.consume(.server, .admin, .xit, repo_opts, io, allocator, &repo, evt.events_ref, &events);
